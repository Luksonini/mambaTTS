...
class StatefulMambaBlock(nn.Module):
    ...
    def forward(self, x, return_state_info=False):
        ...
        if self.stateful_mode and self._hidden_state is not None:
            # Szybka aproksymacja state persistence
            try:
                # Dodaj poprzedni stan jako bias
                prev_state_influence = self._hidden_state.expand(B, L, -1) * 0.1  # Małe wpływ
                x1_processed = x1_ssm * torch.sigmoid(dt) + prev_state_influence
                
                # Update state (prosta średnia z końca sekwencji)
                new_state = x1_processed[:, -5:, :].mean(dim=1, keepdim=True)  # Średnia z ostatnich 5 timestepów
                self._hidden_state = new_state.detach()
                
            except Exception as e:
                logger.warning(f"Fast stateful failed: {e}, using stateless")
                x1_processed = x1_ssm * torch.sigmoid(dt)
        else:
            # Simplified but effective state space (stateless)
            x1_processed = x1_ssm * torch.sigmoid(dt)
...
class StatefulComparisonExperiment:
    ...
    def _process_stateful_proper(self, model, data_loader, optimizer):
        ...
        try:
            ...
            logger.info(f"🔗 DEBUG: Stateful batch - chunk_idx={chunk_idx}, batch_size={batch_size}")
            ...
            return {
                ...
                'avg_state_norm': 0.5,  # Placeholder
